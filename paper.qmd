...
`r if (knitr::is_latex_output()) "## Tables with stargazer()" else ""`

`r if (knitr::is_latex_output()) "@tbl-2 was generated using the stargazer() R package [@hlavac2013stargazer] and shows summary statistics of our data." else ""`
...
`r if (knitr::is_latex_output()) "In the above example we set the 'type' argument to 'latex' (default) which enables us to compile the table for PDF. This way, if you're reading the HTML output of this document, you won't be able to see the table (or it's cross-reference). This means, that dependent on your desired output format you need to change the type argument. For example, use 'type=html' for HTML output formats.<br> Lastly, note, how we specified 'results: asis' into the code chunk options. This is important so that latex is properly printed to the PDF." else ""`
...
```{r echo=TRUE, paged.print=FALSE, results="asis"}
#| label: tbl-6
