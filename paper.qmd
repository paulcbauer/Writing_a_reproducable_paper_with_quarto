...
```{{r}}
install.packages(c("knitr", "kable", "kableExtra",
...
@tbl-2 was generated using the `stargazer()` R package [@hlavac2013stargazer] and shows summary statistics of our data.
...
In the above example we set the `type` argument to `latex` (default) which enables us to compile the table for PDF. This way, if you're reading the HTML output of this document, you won't be able to see the table (or it's cross-reference). This means, that dependent on your desired output format you need tp change the type argument. For example, use `type="html"` for HTML outfput formats.

Lastly, note, how we specified `results: "asis"` into the code chunk options. This is important so that latex is properly printed to the PDF.
...
## R base graphs
Inserting figures can be slightly more complicated. Ideally, we would produce and insert them directly in the `.qmd` file. It's relatively simple to insert R base graphs as you can see in @fig-1.
...
Below in @fig-2 we also show a ggplot2 example.
