...
install.packages(c("knitr", "kableExtra",
                   "stargazer", "modelsummary", "gt", "gtsummary"))
```

```{r eval=F}
install.packages(c("knitr", "kableExtra",
                   "stargazer", "modelsummary", "knitr", "gt", "gtsummary"))
```
...
@tbl-7 shows the output for a regression table. Make sure you name all the models you estimate with unique identifiers (even if its 50 models) and explicitly refer to model names (M1, M2 etc.) in the text.
...
if(knitr::is_html_output()){table7}else{table7 %>% gt::as_latex() %>% as.character() %>% gsub("\\(Intercept\\)", "Intercept", .) %>% cat()}
